...
class CareleastSpectral(CareleastBase):
    ...
    def __init__(self, asu_collection, likelihood, scaling_model, grid_size, unit_cell, n_particles=4, b_factor=20.0, learning_rate=1e-3, friction=0.9, temperatures=None, use_positivity=True, tv_weight=0.0, prior_weight=0.1, enforce_symmetry=True):
        ...
        # Init proportional to Sigma (Cold Start)
        ...
        self.sparse_mode = (not self.use_positivity) and (self.tv_weight <= 0.0)
        if self.sparse_mode:
            print("Careleast: Constraints disabled. Using EXPLICIT VECTORIZED SPARSE Gradient Mode.")
    ...
    def _expand_to_full(self, F_flat_state):
        ...
        F_full = tf.concat([F_half, redundant_part], axis=-1)
        return F_full
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if not self.sparse_mode:
                if self.use_positivity or (self.tv_weight > 0.0):
                    ...
                    rho_complex = tf.signal.ifft3d(F_full)
        ...
        if self.sparse_mode:
            ...
            # 1. Read Momentum (Uses Gather axis=1, AVOIDS GatherNd)
            m_curr_2d = tf.gather(self.momentum, flat_indices, axis=1) # (N, N_obs)
            
            # Flatten for calculation
            m_flat = tf.reshape(m_curr_2d, (-1,))
            g_flat = tf.reshape(grads_F_raw, (-1,))
            
            # 2. Tile Temps and Sigma
            ...
            # 4. Write Back (ScatterNd)
            # Construct indices (N_p * N_obs, 2)
